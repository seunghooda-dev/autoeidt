...
- 실제 작업공간 탭: 상단 `Edit`/`Captions`/`Graphics`/`Export`가 각각 클립 편집, 자막 편집, 방송 그래픽, 납품 포맷·렌더 사전검사 패널로 전환
...
- 세션 자동 복원: 정상 종료 시 마지막 편집을 즉시 저장하고 다음 실행에서 프로젝트와 타임라인을 자동 재개. 비정상 종료로 판별되면 자동 덮어쓰지 않고 `Recover`에서 복구본을 선택
...
- 동적 멀티트랙: 트랙 메뉴에서 비디오 V1~V4와 오디오 A1~A8을 활성화·비활성화하고, V2~V4 잠금·표시와 A3~A8 잠금·음소거, A1~A8 다중 Solo를 트랙별로 독립 제어. 클립이 있는 최상위 트랙은 실수로 제거되지 않도록 보호하며 트랙 수·라우팅·제어 상태는 로컬·백엔드 프로젝트 저장과 Undo/Redo에 손실 없이 보존
- V2~V4 B-roll + A3~A8 연결 오디오: 미디어 빈 드래그 앤 드롭 대상을 V2/V3/V4 중 선택하고 Inspector에서 비디오·오디오 트랙을 재할당. 프레임 단위 이동·양끝 트림·표시·잠금·우클릭 삭제, PIP 위치·크기·회전·투명도, 트랙별 음소거·볼륨·팬·페이드, V2→V4 레이어 순서의 프로그램 프리뷰·최종 렌더 합성 지원
- A3~A8 독립 오디오 클립: WAV/MP3/AAC/FLAC/M4A/OGG/OPUS/WMA를 미디어 빈에서 가져와 원하는 보조 트랙과 재생 헤드에 배치. 타임라인 이동·양끝 트림, Inspector 라우팅·0~200% 볼륨·팬·페이드·음소거·복제·재연결·삭제, Undo/Redo, 프로젝트 복구, Program 프리뷰와 최종 48kHz 스테레오 믹스 지원
- 오디오 볼륨 자동화: V1의 A1/A2, V2~V4 연결 오디오, A3~A8 독립 오디오에 30p 기준 0~200% 볼륨 키프레임을 추가·갱신·삭제·탐색하고 선형 보간. 타임라인 다이아몬드 표시, 트림·분할·Undo/Redo·프로젝트 저장, Program 프리뷰와 최종 FFmpeg 렌더에 동일하게 반영
- G1 방송 그래픽: Lower Third, Headline, Corner Bug/LIVE 프리셋을 재생 헤드에 추가하고 30p 이동·양끝 트림, 잠금·표시·활성화·복제·삭제, 문구·위치·크기·투명도·방송 색상 스와치 편집 지원. 프로젝트 저장·Undo/Redo·Program 프리뷰·최종 FFmpeg 렌더에 동일한 타이밍과 스타일로 반영
...
```text
wav, mp3, aac, flac, m4a, ogg, opus, wma
```
